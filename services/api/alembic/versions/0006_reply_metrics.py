...
def upgrade():
    ...
    with op.batch_alter_table("emails") as batch:
        if "first_user_reply_at" not in columns:
            batch.add_column(sa.Column("first_user_reply_at", sa.DateTime(timezone=True), nullable=True))
        if "last_user_reply_at" not in columns:
            batch.add_column(sa.Column("last_user_reply_at", sa.DateTime(timezone=True), nullable=True))
        if "user_reply_count" not in columns:
            batch.add_column(sa.Column("user_reply_count", sa.Integer(), nullable=False, server_default="0"))
