...
def upgrade() -> None:
    ...
    op.execute("COMMENT ON COLUMN emails.quarantined IS 'Automatically quarantined by security analyzer when risk_score >= threshold'")
    op.execute("COMMENT ON COLUMN emails.flags IS 'Array of security risk flags [{signal, evidence, weight}, ...]'")
