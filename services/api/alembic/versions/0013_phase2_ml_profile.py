...
def upgrade() -> None:
    ...
    op.add_column(
        "emails", sa.Column("event_start_at", sa.DateTime(timezone=True), nullable=True)
    )
    ...
    op.create_index(
        "ix_emails_event_start_at", "emails", ["event_start_at"], unique=False
    )
    op.create_index("ix_emails_due_date", "emails", ["due_date"], unique=False)

    # Add comments for documentation
    op.execute(
        "COMMENT ON COLUMN emails.event_start_at IS 'Start date/time for events, webinars, meetings'"
    )
    op.execute(
        "COMMENT ON COLUMN emails.event_location IS 'Location/venue for events (physical or virtual URL)'"
    )
    op.execute(
        "COMMENT ON COLUMN emails.ml_features IS 'Extracted ML features (TF-IDF, counts, etc.)'"
    )
    op.execute(
        "COMMENT ON COLUMN emails.ml_scores IS 'ML model probability scores per category'"
    )
    op.execute(
        "COMMENT ON COLUMN emails.amount_cents IS 'Bill/invoice amount in cents (USD)'"
    )
    op.execute(
        "COMMENT ON COLUMN emails.due_date IS 'Due date for bills, invoices, payments'"
    )
