...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_oauth_tokens_user_email'), 'oauth_tokens', ['user_email'], unique=False)
    
    # Add new columns to emails table
    op.add_column('emails', sa.Column('gmail_id', sa.String(length=128), nullable=True))
    op.add_column('emails', sa.Column('labels', postgresql.ARRAY(sa.String()), nullable=True))
    op.add_column('emails', sa.Column('label_heuristics', postgresql.ARRAY(sa.String()), nullable=True))
    ...
    op.create_index('idx_emails_gmail_search', 'emails', ['subject', 'from_addr', 'to_addr'], unique=False)
