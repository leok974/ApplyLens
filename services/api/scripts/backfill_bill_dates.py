...
def scan_bills_missing_dates(client) -> Iterable[Dict[str, Any]]:
    ...
    return helpers.scan(
        ...
        query={
            "query": query,
            "_source": ["id", "subject", "body_text", "received_at", "dates", "expires_at"]
        },
        size=BATCH
    )
...
def transform(doc: Dict[str, Any]) -> Dict[str, Any] | None:
    ...
    recv = src.get("received_at") or dt.datetime.utcnow().replace(tzinfo=dt.timezone.utc).isoformat().replace("+00:00", "Z")
