...
def parse_args():
    ...
    parser.add_argument(
        "--csv",
        type=str,
        help="Output CSV file path for mismatches"
    )
    parser.add_argument(
        "--allow",
        type=int,
    ...
    parser.add_argument(
        "--stratify",
        action="store_true",
        help="Stratify sample by category/date"
    )
    ...
    if field in DATE_FIELDS:
        try:
            ...
            if isinstance(db_value, (datetime, date)):
                db_date = db_value.date() if isinstance(db_value, datetime) else db_value
            else:
                db_date = datetime.fromisoformat(str(db_value).replace('Z', '+00:00')).date()
            
            if isinstance(es_value, (datetime, date)):
                es_date = es_value.date() if isinstance(es_value, datetime) else es_value
            else:
                es_date = datetime.fromisoformat(str(es_value).replace('Z', '+00:00')).date()
...
def fetch_db_sample(db, email_ids: List[int], fields: List[str]) -> Dict[int, Dict[str, Any]]:
...
def fetch_es_sample(es_client, email_ids: List[int], fields: List[str], index: str) -> Dict[int, Dict[str, Any]]:
...
def sample_email_ids(db, sample_size: int, stratify: bool = False) -> List[int]:
    ...
    result = db.execute(
        select(Email.id).order_by(func.random()).limit(sample_size)
    ).scalars().all()
...
def generate_report(
    ...
    report = {
        ...
        "summary": {
            ...
            "mismatch_percentage": round(len(mismatches) / len(all_ids) * 100, 2) if all_ids else 0,
