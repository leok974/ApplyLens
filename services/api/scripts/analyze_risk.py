...
SUSPICIOUS_KEYWORDS = [
    ...
    "password", "security alert", "unusual activity", "billing issue",
    "account update", "re-activate", "immediate action"
]
...
RECRUITER_DOMAINS = [
    ...
    "jobvite.com", "smartrecruiters.com", "icims.com"
]
...
def compute_sender_domain_risk(sender: str) -> Tuple[float, Dict[str, Any]]:
    ...
    if domain in RECRUITER_DOMAINS or any(x in domain for x in ["greenhouse", "lever", "workday"]):
        return 10.0, {
            "domain": domain,
            "trust_level": "recruiter",
            "points": 10.0
        }
...
def compute_source_confidence_risk(source_confidence: float) -> Tuple[float, Dict[str, Any]]:
    """
...
def compute_risk_score(email: Email) -> Tuple[float, Dict[str, Any]]:
    ...
    confidence_score, confidence_details = compute_source_confidence_risk(email.source_confidence)
...
def run():
    ...
    while True:
        ...
        print(f"Progress: {total_processed}/{total_count} emails processed, {total_updated} updated")
