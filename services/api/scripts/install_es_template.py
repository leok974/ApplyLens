...
def install_template(es_url: str, template_name: str, template_body: dict) -> bool:
    ...
    try:
        ...
        print(f"   Mappings: {len(template_body['template']['mappings']['properties'])} properties")
...
def main():
    ...
    try:
        ...
        if success:
            ...
            print("2. For existing indices, use: python scripts/update_existing_index_mapping.py")
            print("3. Verify with: curl http://localhost:9200/_index_template/emails-template")
            sys.exit(0)
        else:
            sys.exit(1)
