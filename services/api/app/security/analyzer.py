...
MALICIOUS_KEYWORD_PATTERNS: Tuple[re.Pattern, ...] = tuple(
    ...
    for p in [
        r"invoice\.exe",
        r"payment\s*verification\s*link",
        r"urgent\s*action\s*required",
        r"password\s*reset\s*attachment",
    ]
)
...
class BlocklistProvider:
    ...
    def __init__(self, path: str):
        ...
        self.trusted_domains: Set[str] = {
            d.lower() for d in data.get("trusted_domains", [])
        }
...
class EmailRiskAnalyzer:
    ...
    suspicious_tlds: Set[str] = field(
        default_factory=lambda: set(DEFAULT_SUSPICIOUS_TLDS)
    )
    ...
    def analyze(
        ...
        *,
        ...
        domain_first_seen_days_ago: Optional[int] = None,
    ) -> RiskAnalysis:
        """
        ...
        if _auth_contains(auth_header, "dmarc", "fail", default_none=True):
            flags.append(
                RiskFlag(
                    signal="DMARC_FAIL",
                    evidence=f"auth={auth_header[:120]}",
                    weight=w.DMARC_FAIL,
                )
            )
        ...
        for visible, href in url_pairs:
            ...
            if (
                visible
                and host
                and visible.strip().lower() not in {host.lower(), href.lower()}
            ):
                flags.append(
                    RiskFlag(
                        signal="URL_HOST_MISMATCH",
            ...
            if self.blocklists and host and self.blocklists.is_blocked_host(host):
                flags.append(
                    RiskFlag(
                        signal="BLOCKLISTED_HASH_OR_HOST",
        ...
        for att in attachments or []:
            ...
            if any(mt.startswith(pfx) for pfx in QUARANTINE_MIME_PREFIXES):
                flags.append(
                    RiskFlag(
                        signal="EXECUTABLE_OR_HTML_ATTACHMENT",
                        evidence=f"{fn} ({mt})",
...
def _auth_contains(
    header: str, mechanism: str, needle: str, default_none: bool = False
) -> bool:
    if not header:
        return (
            False if not default_none else True
        )  # treat missing as weak signal only when requested
    h = header.lower()
...
def _looks_like_brand_mismatch(display_name: str, domain: str) -> bool:
    ...
    name_tokens = {
        t for t in name_tokens if len(t) >= 3 and t not in {"inc", "llc", "co", "the"}
    }
