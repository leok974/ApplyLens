...
def parse_due_cutoff(
    ...
    for name, wd in WEEKDAY.items():
        if f"before {name}" in text_l or f"by {name}" in text_l:
            ...
            return cutoff_local.astimezone(dt.timezone.utc).isoformat().replace("+00:00", "Z")
    ...
    if m:
        ...
        try:
            ...
            return dt_local.astimezone(dt.timezone.utc).isoformat().replace("+00:00", "Z")
        except ValueError:
