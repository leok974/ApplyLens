...
RISK_INDICATORS = {
    ...
    "credential_phishing": (r"\b(verify credentials|confirm password|update payment)\b", 25),
    "external_sender": (r"@(?!.*?(gmail|yahoo|outlook|hotmail))", 5),  # Not major email providers
}
...
def calculate_risk_score(email: Dict[str, Any]) -> float:
    ...
    if sender and "@" in sender:
        ...
        if any(brand in sender_lower for brand in ["paypal", "amazon", "bank", "irs"]):
            if not any(brand in sender_lower.split("@")[1] for brand in ["paypal", "amazon"]):
...
def extract_expiry_date(email: Dict[str, Any]) -> Optional[datetime]:
    ...
    if dates and isinstance(dates, list):
        future_dates = [
            d for d in dates 
            if isinstance(d, (datetime, str)) and 
            (datetime.fromisoformat(d) if isinstance(d, str) else d) > datetime.now()
        ]
        if future_dates:
...
def extract_profile_tags(email: Dict[str, Any], user_interests: List[str] = None) -> List[str]:
    """
    ...
    # Urgency detection
    if re.search(r"\b(urgent|expires today|last chance|ending soon)\b", combined, re.IGNORECASE):
        tags.append("urgent")
...
def classify_email(email: Dict[str, Any], user_profile: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
...
def classify_batch(emails: List[Dict[str, Any]], user_profile: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    """
