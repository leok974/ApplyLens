...
def _coerce_date_token(tok: str, received_at_utc: dt.datetime) -> Optional[dt.datetime]:
    ...
    m = re.match(r"^(?:(?:by|on)\s+)?([A-Za-z]{3,})\s+(\d{1,2})(?:\s+(\d{4}))?$", t, re.I)
    if m:
...
def extract_due_dates_from_subject(subject: str, received_at_utc: dt.datetime) -> List[str]:
    """
...
def is_bill_related(subject: str, body_text: str) -> bool:
    ...
    bill_keywords = [
        "bill", "invoice", "statement", "payment", "amount due",
        "account due", "balance due", "pay by", "payment reminder",
        "overdue", "past due", "payment required"
    ]
