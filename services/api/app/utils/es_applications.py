...
def es_tombstone_application(app_id: int):
    ...
    if not es_available():
        logger.debug(
            f"[ES Sync] Elasticsearch not available, skipping tombstone for app {app_id}"
        )
        return
    ...
    try:
        ...
        es.update(
            index=index,
            id=str(app_id),
            body={"doc": {"visible": False, "archived": True}},
...
def es_upsert_application(app):
    ...
    if not es_available():
        logger.debug(
            f"[ES Sync] Elasticsearch not available, skipping upsert for app {app.id}"
        )
        return
...
def es_delete_application(app_id: int):
    ...
    if not es_available():
        logger.debug(
            f"[ES Sync] Elasticsearch not available, skipping delete for app {app_id}"
        )
        return
