...
def _parts_to_extractable(msg: Dict[str, Any]) -> Extractable:
    ...
    for p in parts:
        ...
        if filename and size and not re.search(r"text/(plain|html)", mime_type, re.IGNORECASE):
            attachments.append({
                "filename": filename,
                "mimeType": mime_type,
                "size": size,
    ...
    class SingleUserProvider:
        async def fetch_thread_latest(
            ...
            try:
                ...
                enriched = await _maybe_parse_pdf_text(svc, settings.GMAIL_USER, latest, base)
            ...
            if user_email:
                ...
                if tok and settings.GMAIL_CLIENT_ID and settings.GMAIL_CLIENT_SECRET:
                    try:
                        ...
                        # Optional PDF parsing
                        enriched = await _maybe_parse_pdf_text(svc, user_email, latest, base)
