...
try:
    ...
    logger.warning(
        "Issue adapters not available - incidents will be created without external issues"
    )
...
class InvariantWatcher:
    ...
    def check_invariants(self, lookback_minutes: int = 60) -> List[Incident]:
        ...
        results = (
            ...
            .filter(
                EvalResult.created_at >= since, EvalResult.invariant_results.isnot(None)
            )
    ...
    def _is_rate_limited(self, kind: str, key: str, hours: int = 1) -> bool:
        ...
        count = (
            self.db.query(Incident)
            .filter(
                Incident.kind == kind, Incident.key == key, Incident.created_at >= since
            )
            .count()
        )
    ...
    def _create_external_issue(self, incident: Incident) -> Optional[str]:
        ...
        try:
            ...
            if response.success:
                logger.info(
                    f"Created external issue for incident {incident.id}: {response.issue_url}"
                )
    ...
    def _create_invariant_incident(
        self, eval_result: EvalResult, inv_result: Dict[str, Any]
    ) -> Incident:
    ...
    def _create_budget_incident(
        self, key: str, budget_data: Dict[str, Any]
    ) -> Incident:
    ...
    def _create_planner_incident(
        self, version: str, canary_data: Dict[str, Any]
    ) -> Incident:
