...
def ensure_user(
    db: Session, email: str, name: str | None = None, demo: bool = False
) -> User:
    """Find or create a user by email."""
    user = db.query(User).filter_by(email=email).first()
    if not user:
        user = User(id=str(uuid.uuid4()), email=email, name=name, is_demo=demo)
        db.add(user)
        db.commit()
        db.refresh(user)
...
def run(db: Session, leo_email: str, make_demo: bool):
    ...
    for token in orphaned_tokens:
        ...
        if token.user_email == leo_email:
            ...
            print(
                f"Warning: Unknown email {token.user_email}, assigning to {leo_email}"
            )
            token.user_id = leo.id
            updated_tokens += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--make-demo", action="store_true", help="Mark demo user as demo"
    )
