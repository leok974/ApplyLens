...
class MetricsStore:
    ...
    def window_stats(
        ...
        """
        # Query recent runs
        query = self.db.query(AgentAuditLog).filter(
            AgentAuditLog.status == "succeeded"
        )
...
class RegressionDetector:
    ...
    def evaluate(self, window_runs: int = 100) -> Dict[str, Any]:
        ...
        if stats["v2"]["samples"] < REGRESS_MIN_SAMPLE:
            logger.info(f"Insufficient V2 samples: {stats['v2']['samples']} < {REGRESS_MIN_SAMPLE}")
            return {
                "action": "none",
        ...
        if quality_drop > REGRESS_MAX_QUALITY_DROP:
            breaches.append(f"quality (V1: {stats['v1']['quality']:.1f}, V2: {stats['v2']['quality']:.1f}, drop: {quality_drop:.1f})")
        
        # Latency regression (V2 p95 exceeds threshold)
        if stats["v2"]["latency_p95_ms"] > REGRESS_MAX_LATENCY_P95_MS:
            breaches.append(f"latency (V2 p95: {stats['v2']['latency_p95_ms']:.1f}ms > {REGRESS_MAX_LATENCY_P95_MS}ms)")
        
        # Cost regression (V2 exceeds threshold)
        if stats["v2"]["cost_cents"] > REGRESS_MAX_COST_CENTS:
            breaches.append(f"cost (V2: {stats['v2']['cost_cents']:.3f}¢ > {REGRESS_MAX_COST_CENTS}¢)")
