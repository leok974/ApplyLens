...
async def get_agent_metrics(
    ...
    latencies = [m.avg_latency_ms * m.total_runs for m in metrics if m.avg_latency_ms]
    ...
    satisfaction = (
        thumbs_up / (thumbs_up + thumbs_down) if (thumbs_up + thumbs_down) > 0 else None
    )
...
async def get_all_agents_health(
    ...
    for agent in agents:
        ...
        quality_samples = sum(
            m.quality_samples for m in metrics if m.quality_samples > 0
        )
        avg_quality = (
            sum(quality_scores) / quality_samples if quality_samples > 0 else None
        )
...
async def run_redteam_evaluation(
    agent: Optional[str] = Query(
        None, description="Specific agent to test (default: all)"
    ),
):
    """
    ...
    failed_task_ids = [
        r.task_id for r in run.results if not r.success or r.quality_score < 70.0
    ]
