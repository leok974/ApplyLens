...
def get_es():
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail=f"Failed to connect to Elasticsearch: {e}"
        )
...
class ActionItem(BaseModel):
    ...
    action: str = Field(..., description="Action type (archive_email, unsubscribe_via_header, etc.)")
...
class ChatResponse(BaseModel):
    ...
    citations: List[Citation] = Field(default_factory=list, description="Email sources used")
                ...
                try:
                    ...
                    for brand in brands[:5]:  # Cap at 5 brands
                        pol = Policy(
                            ...
                            condition={"all": [{"eq": ["category", "promo"]}, {"regex": ["sender", brand]}]}
                        )
                        db.add(pol)
                    db.commit()
                    db.close()
