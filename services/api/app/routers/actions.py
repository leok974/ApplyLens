...
class ApproveRequest(BaseModel):
    ...
    screenshot_data_url: Optional[str] = Field(None, description="Base64 data URL of screenshot")
...
def build_rationale(
    ...
    # Build narrative
    narrative = {
        "summary": f"Policy '{policy.name}' matched this email."
    }
    
    if policy.action == ActionType.archive_email and features.get("expired_days", 0) > 0:
        narrative["summary"] = f"Email expired {features['expired_days']} days ago; auto-archiving per policy."
    elif policy.action == ActionType.quarantine_attachment and features.get("risk_score", 0) >= 80:
        narrative["summary"] = f"High risk score ({features['risk_score']:.1f}); quarantining per policy."
...
def propose_actions(
    ...
    return {
        "created": [pa.id for pa in created],
        "count": len(created)
    }
...
def update_policy(
    ...
    if req.name is not None:
        # Check for duplicate name
        existing = db.query(Policy).filter(Policy.name == req.name, Policy.id != policy_id).first()
        if existing:
            raise HTTPException(400, f"Policy with name '{req.name}' already exists")
