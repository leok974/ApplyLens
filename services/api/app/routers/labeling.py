...
def label_rebuild(
    limit: int = 2000, user_email: Optional[str] = None, db: Session = Depends(get_db)
):
    """
            ...
            if es_client:
                ...
                for email_row in emails:
                    if hasattr(email_row, "category") and email_row.gmail_id:
                        ...
                        bulk_body.append(
                            {"update": {"_index": ES_INDEX, "_id": email_row.gmail_id}}
                        )
                        ...
                        if (
                            hasattr(email_row, "event_start_at")
                            and email_row.event_start_at
                        ):
                            doc["event_start_at"] = email_row.event_start_at.isoformat()
                        if (
                            hasattr(email_row, "event_location")
                            and email_row.event_location
                        ):
                ...
                if bulk_body:
                    ...
                    if response.get("errors"):
                        ...
                        logger.warning(
                            f"ES bulk update had errors (processed {es_synced} docs)"
                        )
                    else:
    ...
    return [
        {
            ...
            "event_start_at": (
                e.event_start_at.isoformat() if e.event_start_at else None
            ),
...
def get_labeling_stats(user_email: Optional[str] = None, db: Session = Depends(get_db)):
    ...
    """
    query = db.query(Email.category, func.count(Email.id).label("count")).group_by(
        Email.category
    )
