...
async def recompute_all_risk(
    background_tasks: BackgroundTasks, dry_run: bool = False, batch_size: int = 500
):
    """
    ...
    try:
        ...
        if result.returncode != 0:
            raise HTTPException(
                status_code=500, detail=f"Risk recomputation failed: {result.stderr}"
            )
        ...
        raise HTTPException(
            status_code=504, detail="Risk recomputation timed out after 10 minutes"
        )
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error running risk recomputation: {str(e)}"
        )


@router.get("/risk-summary")
async def get_risk_summary(
    category: str = None, days: int = 7, db: Session = Depends(get_db)
):
    """
    ...
    for level, min_score, max_score in risk_levels:
        count_stmt = select(func.count(Email.id)).where(
            and_(*filters, Email.risk_score >= min_score, Email.risk_score < max_score)
        )
...
async def get_risk_trends(
    days: int = 30, granularity: str = "day", db: Session = Depends(get_db)
):
    """
...
async def automation_health(db: Session = Depends(get_db)):
    ...
    last_computed_stmt = (
        select(Email.features_json)
        .where(Email.features_json.isnot(None))
    ...
    return {
        ...
        "statistics": {
            ...
            "coverage_percentage": round(
                (emails_with_scores / total_emails * 100) if total_emails > 0 else 0, 2
            ),
        },
        "last_computed": last_computed,
        "recommendations": (
            ["Run /automation/recompute to compute risk scores"]
            if emails_with_scores == 0
            else []
        ),
    }
