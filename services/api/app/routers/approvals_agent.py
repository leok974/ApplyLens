...
def verify_signature(
    request_id: str, nonce: str, signature: str, secret_key: str
) -> bool:
    """
...
def list_approvals(
    ...
    approved = (
        db.query(AgentApproval).filter(AgentApproval.status == "approved").count()
    )
    rejected = (
        db.query(AgentApproval).filter(AgentApproval.status == "rejected").count()
    )

    # Get paginated results
    approvals = (
        query.order_by(AgentApproval.requested_at.desc())
        .offset(offset)
        .limit(limit)
        .all()
    )
...
def get_approval(
    ...
    """
    approval = (
        db.query(AgentApproval).filter(AgentApproval.request_id == request_id).first()
    )
...
def verify_approval_signature(
    request_id: str, body: ApprovalSignature, db: Session = Depends(get_db)
):
    """
