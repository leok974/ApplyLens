...
def intent_and_params(text: str) -> Dict[str, Any]:
    ...
    if any(
        word in t
        for word in ["suspicious", "fishy", "malware", "phish", "spam", "risky"]
    ):
...
async def run(query: NLQuery):
    ...
    if parsed["intent"] == "clean_promos":
        ...
        return {"intent": "show_suspicious", "emails": emails, "count": len(emails)}
            ...
            items.append(
                {
                    ...
                    "title": (
                        e.get("subject")
                        or f"Bill from {e.get('sender_domain', 'unknown')}"
                    )
                    + money_str,
                    "due_at": due_at,
