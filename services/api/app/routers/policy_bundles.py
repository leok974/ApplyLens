...
class PolicyBundleCreate(BaseModel):
    ...
    rules: list[dict[str, Any]] = Field(..., min_items=1, description="Array of policy rules")
    notes: str | None = Field(None, max_length=512, description="Release notes")
    created_by: str = Field(..., min_length=1, max_length=128, description="Author email or username")
...
class PolicyBundleUpdate(BaseModel):
    ...
    @validator("rules")
    def validate_rules(cls, rules: list[dict[str, Any]] | None) -> list[dict[str, Any]] | None:
        """Validate each rule against JSON schema."""
        if rules is None:
            return None
...
async def list_bundles(
    ...
    if active_only:
        query = query.where(PolicyBundle.active == True)
...
async def get_active_bundle(db: AsyncSession = Depends(get_db)) -> PolicyBundleResponse | None:
    """
    Get the currently active policy bundle.
    
    Returns the bundle with active=True, or None if no active bundle.
    """
    query = select(PolicyBundle).where(PolicyBundle.active == True)
    result = await db.execute(query)
...
async def get_bundle_by_version(
    ...
    if not bundle:
        raise HTTPException(status_code=404, detail=f"Policy bundle version {version} not found")
...
async def create_bundle(
    ...
    if existing.scalar_one_or_none():
        raise HTTPException(status_code=409, detail=f"Version {bundle_data.version} already exists")
...
async def update_bundle(
    bundle_id: int,
    bundle_data: PolicyBundleUpdate,
    db: AsyncSession = Depends(get_db)
) -> PolicyBundleResponse:
    """
...
async def delete_bundle(
    ...
    if bundle.active or bundle.canary_pct > 0:
        raise HTTPException(
            status_code=409,
            detail="Cannot delete active or canary bundle"
        )
