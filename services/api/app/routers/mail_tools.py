...
class Action(BaseModel):
    ...
    params: Optional[Dict[str, Any]] = Field(
        None, description='Action-specific parameters (e.g., {"label": "important"})'
    )
    policy_id: Optional[str] = Field(
        None, description="Policy that triggered this action"
    )
...
def check_action_safety(
    action: Action, email: Optional[Email] = None
    ...
    if action.action in HIGH_RISK_ACTIONS:
        if not action.confidence or action.confidence < 0.8:
            return (
                False,
                f"High-risk action '{action.action}' requires confidence >= 0.8",
                warnings,
            )
    ...
    if email is None:
        warnings.append(
            "Email not found in database - action may fail during execution"
        )
...
async def preview_actions(actions: List[Action], db: AsyncSession = Depends(get_db)):
    """
...
async def execute_actions(
    ...
    return ActionExecuteResult(applied=applied, failed=failed, results=results_list)


async def _execute_single_action(
    action: Action, email: Optional[Email], db: AsyncSession
) -> bool:
    """
...
async def get_action_history(email_id: str, db: AsyncSession = Depends(get_db)):
    """
    ...
    stmt = (
        select(ActionsAudit)
        .where(ActionsAudit.email_id == email_id)
        .order_by(ActionsAudit.created_at.desc())
    )
    result = await db.execute(stmt)
...
async def suggest_actions(email_ids: List[str], db: AsyncSession = Depends(get_db)):
    """
