...
router = APIRouter(prefix="/api/warehouse/profile", tags=["warehouse", "analytics"])
...
async def top_senders(limit: int = 10) -> list[dict[str, Any]]:
    ...
    return await cache_json(cache_key, ttl=60, fn=lambda: warehouse.mq_top_senders_30d(limit))
...
async def activity_daily(days: int = 14) -> list[dict[str, Any]]:
    ...
    return await cache_json(cache_key, ttl=60, fn=lambda: warehouse.mq_activity_daily(days))
...
async def categories_30d(limit: int = 10) -> list[dict[str, Any]]:
    ...
    return await cache_json(cache_key, ttl=60, fn=lambda: warehouse.mq_categories_30d(limit))
