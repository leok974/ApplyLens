...
class SSEPublisher:
    ...
    async def publish(self, event: Dict[str, Any]):
        ...
        logger.info(f"Publishing SSE event: {event['event']} (subscribers: {len(self.subscribers)})")
    ...
    async def subscribe(self, subscriber_id: str) -> asyncio.Queue:
        ...
        logger.info(f"New SSE subscriber: {subscriber_id} (total: {len(self.subscribers)})")
        return queue
    
    def unsubscribe(self, subscriber_id: str):
        """Unsubscribe from events."""
        self.subscribers.pop(subscriber_id, None)
        logger.info(f"SSE subscriber left: {subscriber_id} (remaining: {len(self.subscribers)})")
