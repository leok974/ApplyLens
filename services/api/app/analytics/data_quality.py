...
class DataQualityMonitor:
    ...
    def check_table_quality(self, table_name: str) -> List[QualityCheckResult]:
        ...
        if table_name not in self.quality_configs:
            raise ValueError(
                f"Table {table_name} not registered for quality monitoring"
            )
    ...
    def _check_nulls(self, table_name: str, column_name: str) -> QualityCheckResult:
        ...
        status = (
            QualityCheckStatus.PASSED if failed_rows == 0 else QualityCheckStatus.FAILED
        )
    ...
    def get_quality_summary(self, days: int = 7) -> Dict:
        ...
        warnings = sum(
            1 for r in recent_results if r.status == QualityCheckStatus.WARNING
        )
...
"""
        ...
        # Recent failures
        recent_failures = [
            r for r in self.check_results[-20:] if r.status == QualityCheckStatus.FAILED
        ]
