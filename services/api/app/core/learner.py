...
def featureize(email: Any) -> List[str]:
    ...
    # Category feature
    category = getattr(email, "category", None) or (
        email.get("category") if isinstance(email, dict) else None
    )
    if category:
        feats.append(f"category:{category}")

    # Sender domain feature
    sender_domain = getattr(email, "sender_domain", None) or (
        email.get("sender_domain") if isinstance(email, dict) else None
    )
    if sender_domain:
        feats.append(f"sender_domain:{sender_domain}")

    # List ID feature
    list_id = getattr(email, "list_id", None) or (
        email.get("list_id") if isinstance(email, dict) else None
    )
    if list_id:
    ...
    subject = getattr(email, "subject", "") or (
        email.get("subject", "") if isinstance(email, dict) else ""
    )
...
def get_user_preferences(
    db: Session, user_id: str, top_k: int = 20
) -> List[Dict[str, Any]]:
    """
