...
def clean_promos(rag: Dict[str, Any], user_text: str) -> Tuple[str, List[dict]]:
    ...
    for doc in candidates:
        ...
        is_exception = any(
            exc in sender or exc in sender_domain
    ...
    if not actions:
        if exceptions:
            return (f"No old promos found (respecting exceptions: {', '.join(exceptions)}).", [])
        return ("No promotional emails older than a week found.", [])
    
    exception_note = f" (respecting exceptions: {', '.join(exceptions)})" if exceptions else ""
...
def unsubscribe_inactive(rag: Dict[str, Any], user_text: str) -> Tuple[str, List[dict]]:
    ...
    inactive = [
        ...
        if (d.get("received_at") or "") < cutoff_iso
    ]
...
def create_calendar_events(rag: Dict[str, Any], user_text: str) -> Tuple[str, List[dict]]:
    """
...
def follow_up(rag: Dict[str, Any], user_text: str) -> Tuple[str, List[dict]]:
    ...
    for doc in rag["docs"]:
        ...
        if any(kw in sender or kw in subject for kw in ["recruiter", "hiring", "interview", "application"]):
