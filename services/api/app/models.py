from sqlalchemy import Column, Integer, String, DateTime, Text, JSON, Index, ForeignKey, Enum, Float, BigInteger, TIMESTAMP, text, Boolean
...
class Email(Base):
    ...
    flags = Column(JSONB, nullable=False, default=list, server_default=text("'[]'::jsonb"))  # Security risk flags [{signal, evidence, weight}, ...]
    quarantined = Column(Boolean, nullable=False, default=False, server_default=text("false"))  # Automatically quarantined by security analyzer
...
class SecurityPolicy(Base):
    ...
    auto_quarantine_high_risk = Column(Boolean, nullable=False, default=True, server_default=text("true"))
    auto_archive_expired_promos = Column(Boolean, nullable=False, default=True, server_default=text("true"))
    auto_unsubscribe_enabled = Column(Boolean, nullable=False, default=False, server_default=text("false"))
    auto_unsubscribe_threshold = Column(Integer, nullable=False, default=10, server_default="10")
...
class ProposedAction(Base):
    ...
    params = Column(JSON, default=dict)  # Action-specific parameters (label name, folder, etc.)
    ...
    status = Column(String, default="pending", nullable=False)  # pending|approved|rejected|executed|failed
...
class AuditAction(Base):
    ...
    why = Column(JSON, default=dict)  # Same schema as rationale (features, aggs, narrative)
