...
class AgentSettings(BaseSettings):
    ...
    GCP_PROJECT: str | None = None  # GCP project ID (fallback to BQ_PROJECT or "applylens-app")
    GCP_BQ_LOCATION: str = "US"  # BigQuery dataset location
    GCP_CREDENTIALS_PATH: str | None = None  # Path to service account JSON (uses ADC if not set)
    
    # Archive & Auto-Delete Retention Policies
    AUTO_ARCHIVE_REJECTED_AFTER_DAYS: int = 14  # Auto-archive rejected apps after X days
    ...
    OAUTH_REDIRECT_URI: str = "http://localhost:5175/auth/google/callback"  # OAuth callback URL
