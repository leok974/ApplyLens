...
class ChaosInjector:
    ...
    def inject(self, chaos_type: ChaosType, **kwargs):
        ...
        try:
            ...
            metrics.average_impact_ms = (
                (metrics.average_impact_ms * (metrics.injections_successful - 1) + impact_ms)
    ...
    def _execute_injection(self, config: ChaosConfig):
        ...
        if config.chaos_type == ChaosType.API_OUTAGE:
            ...
            raise ChaosException(
                config.error_message or f"Chaos: Rate limit exceeded (HTTP {status_code})"
            )
        
        elif config.chaos_type == ChaosType.TIMEOUT:
            # Simulate timeout
            raise TimeoutError(
                config.error_message or "Chaos: Request timeout"
            )
        
        elif config.chaos_type == ChaosType.NETWORK_PARTITION:
            # Simulate network partition
            raise ConnectionError(
                config.error_message or "Chaos: Network partition"
            )
    ...
    def get_metrics(self, chaos_type: Optional[ChaosType] = None) -> Dict[str, ChaosMetrics]:
    ...
    def generate_report(self) -> str:
        ...
        for chaos_type, metrics in self._metrics.items():
            ...
            lines.append(f"  Injections: {metrics.injections_successful}/{metrics.injections_attempted}")
            ...
            lines.append(f"  Recovery Attempts: {metrics.recovery_successes}/{metrics.recovery_attempts}")
            lines.append("")
        ...
        while True:
            ...
            for experiment in self._scheduled_experiments:
                if now >= experiment["next_run"]:
                    ...
                    logger.info(f"Running chaos experiment: {experiment['chaos_type'].value}")
                    ...
                    experiment["next_run"] = now + timedelta(seconds=experiment["interval_seconds"])
