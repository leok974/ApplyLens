...
class LabeledExample(Base):
    ...
    )  # inbox_triage, knowledge_update, etc.
    key = Column(String(256), nullable=False, index=True)  # thread_id, rule_id, task_id

    # Labeled data
    payload = Column(
        JSON, nullable=False
    )  # Feature snapshot: {spf, dkim, keywords, etc.}
    label = Column(
        String(64), nullable=False, index=True
    )  # High-Risk, Offer, Safe, etc.

    # Provenance
    source = Column(
        String(32), nullable=False, index=True
    )  # approvals|feedback|gold|synthetic
    source_id = Column(
        String(128), nullable=True
    )  # Original record ID (approval_id, feedback_id, etc.)
    ...
    )  # 0-100 confidence in label (if available)
