...
def analyze_planner_diffs(
    ...
    runs = db.query(AgentAuditLog).filter(
    ...
    agent_change_rate = (sum(1 for d in diffs) / total_runs * 100) if total_runs > 0 else 0.0
...
def generate_weekly_report(analysis: Dict[str, Any]) -> str:
    ...
    if analysis['v2_pct'] < 5:
        lines.append("- ✅ **Increase canary to 10%** - V2 traffic is very low, consider ramping up")
    elif analysis['v2_pct'] < 20:
        lines.append("- ✅ **Maintain current split** - Good canary percentage for evaluation")
    else:
        lines.append("- ⚠️ **High canary traffic** - Consider gradual rollout to avoid risk")
    
    if analysis['agent_change_rate'] > 30:
        lines.append("- ⚠️ **High disagreement rate** - V1 and V2 choosing different agents frequently")
