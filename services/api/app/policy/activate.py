...
class CanaryGate:
    ...
    def check(self, metrics: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        if error_rate > self.max_error_rate:
            failures.append(f"Error rate too high: {error_rate:.2%} > {self.max_error_rate:.2%}")
        
        # Check deny rate
        denies = metrics.get("deny_count", 0)
        deny_rate = denies / total if total > 0 else 0
        if deny_rate > self.max_deny_rate:
            failures.append(f"Deny rate too high: {deny_rate:.2%} > {self.max_deny_rate:.2%}")
...
def activate_bundle(
    ...
    current_active = db.query(PolicyBundle).filter(
        PolicyBundle.active == True
    ).first()
...
def rollback_bundle(
    ...
    previous = db.query(PolicyBundle).filter(
        PolicyBundle.id != bundle_id,
        PolicyBundle.activated_at.isnot(None)
    ).order_by(desc(PolicyBundle.activated_at)).first()
