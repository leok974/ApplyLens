...
class GateEvaluator:
    ...
    def evaluate_agent(
        ...
        # Quality score
        if budget.min_quality_score is not None and current_metrics.get(
            "avg_quality_score"
        ):
        ...
        # Latency (avg)
        if budget.max_avg_latency_ms is not None and current_metrics.get(
            "avg_latency_ms"
        ):
        ...
        # Latency (P95)
        if budget.max_p95_latency_ms is not None and current_metrics.get(
            "p95_latency_ms"
        ):
        ...
        # Latency (P99)
        if budget.max_p99_latency_ms is not None and current_metrics.get(
            "p99_latency_ms"
        ):
        ...
        # Cost
        if budget.max_avg_cost_weight is not None and current_metrics.get(
            "avg_cost_per_run"
        ):
        ...
        if baseline_metrics:
            ...
            if current_metrics.get("avg_quality_score") and baseline_metrics.get(
                "avg_quality_score"
            ):
                drop = (
                    baseline_metrics["avg_quality_score"]
                    - current_metrics["avg_quality_score"]
                )
                if drop > 10.0:
                    violations.append(
                        BudgetViolation(
            ...
            if current_metrics.get("avg_latency_ms") and baseline_metrics.get(
                "avg_latency_ms"
            ):
                increase_pct = (
    ...
    def _get_avg_metrics(
        ...
        # Weighted averages
        quality_scores = [
            m.avg_quality_score for m in metrics if m.avg_quality_score is not None
        ]
        latencies_avg = [
            m.avg_latency_ms for m in metrics if m.avg_latency_ms is not None
        ]
        latencies_p95 = [
            m.p95_latency_ms for m in metrics if m.p95_latency_ms is not None
        ]
        latencies_p99 = [
            m.p99_latency_ms for m in metrics if m.p99_latency_ms is not None
        ]
        ...
        return {
            ...
            "avg_latency_ms": sum(latencies_avg) / len(latencies_avg)
            if latencies_avg
            else None,
            "p95_latency_ms": sum(latencies_p95) / len(latencies_p95)
            if latencies_p95
            else None,
            "p99_latency_ms": sum(latencies_p99) / len(latencies_p99)
            if latencies_p99
            else None,
...
def format_gate_report(evaluation: Dict[str, Any]) -> str:
    ...
    if "results" in evaluation:
        ...
        lines.append(
            f"**Violations**: {evaluation['total_violations']} total, {evaluation['critical_violations']} critical\n"
        )
