...
class MockAgentExecutor:
    ...
    def execute(
        self, agent: str, objective: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _mock_inbox_output(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        has_suspicious_domain = (
            any(tld in sender for tld in [".net", ".biz", ".info"])
            and "suspicious" in sender
        )

        if (
            is_phishing
            or has_suspicious_domain
            or context.get("domain_age_days", 1000) < 30
        ):
            risk_level = "high"
            ...
            category = "offer"
            confidence = 0.85
        elif context.get("excessive_caps", False) or context.get(
    ...
    def _mock_insights_output(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "data_quality_warning": not has_historical
            or context.get("data_points", 100) < 10,
...
class EvalRunner:
    ...
    def __init__(
        self, use_mock_executor: bool = True, output_dir: Optional[Path] = None
    ):
        """
    ...
    def run_suite(self, suite: EvalSuite) -> EvalRun:
        ...
        for task in suite.tasks:
            ...
            print(
                f"  {status} {task.id}: score={result.quality_score:.1f}, latency={result.latency_ms:.0f}ms"
            )
    ...
    def _export_run(self, run: EvalRun) -> None:
        ...
        filename = (
            f"{run.agent}_{run.run_id}_{run.timestamp.strftime('%Y%m%d_%H%M%S')}.jsonl"
        )
