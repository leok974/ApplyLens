...
class RiskAccuracyJudge(Judge):
    ...
    def score(self, task: EvalTask, output: Dict[str, Any]) -> tuple[float, str]:
        ...
        if expected_phishing == actual_phishing:
            ...
            reasons.append(
                f"Phishing detection wrong: expected {expected_phishing}, got {actual_phishing}"
            )
        ...
        if expected_category and actual_category == expected_category:
            ...
            score += 10
            reasons.append(
                f"Category wrong: expected {expected_category}, got {actual_category}"
            )
        else:
            score += 10
...
class SyncAccuracyJudge(Judge):
    ...
    def score(self, task: EvalTask, output: Dict[str, Any]) -> tuple[float, str]:
        ...
        if actual_synonyms == expected_synonyms:
            ...
            reasons.append(
                f"Synonym preservation issue: expected {expected_synonyms}, got {actual_synonyms}"
            )
        ...
        if actual_conflicts == expected_conflicts:
            ...
            score += 10
            reasons.append(
                f"Conflict resolution partial: {actual_conflicts}/{expected_conflicts}"
            )
...
class WarehouseHealthJudge(Judge):
    ...
    def score(self, task: EvalTask, output: Dict[str, Any]) -> tuple[float, str]:
        ...
        if actual_healthy == expected_healthy:
            ...
            reasons.append(
                f"Health status wrong: expected {expected_healthy}, got {actual_healthy}"
            )
        ...
        if actual_issues == expected_issues:
            ...
            if diff <= 1:
                score += 30
                reasons.append(
                    f"Issues detection close: {actual_issues} vs {expected_issues}"
                )
            else:
                score += 10
                reasons.append(
                    f"Issues detection off: {actual_issues} vs {expected_issues}"
                )
        ...
        if actual_parity == expected_parity:
            ...
            reasons.append(
                f"Parity check wrong: expected {expected_parity}, got {actual_parity}"
            )
...
class NoFalseNegativesPhishingInvariant(Invariant):
    ...
    def check(self, task: EvalTask, output: Dict[str, Any]) -> tuple[bool, str]:
        ...
        if actual_phishing or risk_level in ["high", "critical"]:
            return (
                True,
                f"Phishing detected: is_phishing={actual_phishing}, risk={risk_level}",
            )

        return (
            False,
            f"False negative: missed phishing (is_phishing={actual_phishing}, risk={risk_level})",
        )
...
class SyncCompletionInvariant(Invariant):
    ...
    def check(self, task: EvalTask, output: Dict[str, Any]) -> tuple[bool, str]:
        ...
        if completion_rate >= 0.95:
            return (
                True,
                f"Sync completion: {actual_synced}/{expected_synced} ({completion_rate:.1%})",
            )

        return (
            False,
            f"Sync incomplete: {actual_synced}/{expected_synced} ({completion_rate:.1%})",
        )
...
class WarehouseParityInvariant(Invariant):
    ...
    def check(self, task: EvalTask, output: Dict[str, Any]) -> tuple[bool, str]:
        ...
        if not actual_parity or not is_healthy:
            return (
                True,
                f"Parity issue detected: parity_ok={actual_parity}, is_healthy={is_healthy}",
            )

        return (
            False,
            f"Failed to detect parity issue: parity_ok={actual_parity}, is_healthy={is_healthy}",
        )
