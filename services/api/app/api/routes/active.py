...
def check_canary_performance(
    ...
    result = guard.check_canary_performance(
        agent=agent,
...
def pause_active_learning(
    ...
    reason_setting = RuntimeSetting(
        key="active_learning.pause_reason",
        value=reason,
        category="active_learning"
    )
    
    existing_reason = db.query(RuntimeSetting).filter_by(key="active_learning.pause_reason").first()
    if existing_reason:
...
def get_active_learning_status(db: Session = Depends(get_db)):
    ...
    # Check if paused
    paused_setting = db.query(RuntimeSetting).filter_by(key="active_learning.paused").first()
