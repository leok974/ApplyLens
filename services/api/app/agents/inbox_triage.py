...
class RiskScorer:
    ...
    SUSPICIOUS_KEYWORDS = {
        ...
        'confirm', 'password', 'billing', 'expired', 'act now',
        'winner', 'congratulations', 'prize', 'claim', 'refund'
    }
    
    # Known safe domains (allowlist)
    SAFE_DOMAINS = {
        'google.com', 'github.com', 'microsoft.com', 'apple.com',
        'amazon.com', 'stripe.com', 'atlassian.com', 'slack.com'
    }
...
class InboxTriageAgent:
    ...
    def plan(self, objective: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if quarantine_threshold < 100:
            steps.append(f"5. Quarantine emails with risk >= {quarantine_threshold} (requires approval)")
    ...
    def execute(self, plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            'total_emails': len(emails),
            'by_risk_level': {k: len(v) for k, v in by_risk_level.items()},
            'high_risk_count': len(by_risk_level['HIGH']) + len(by_risk_level['CRITICAL']),
        ...
        for level in ['CRITICAL', 'HIGH', 'MEDIUM', 'LOW', 'SAFE']:
            ...
            if count > 0:
                ...
                if level in ['CRITICAL', 'HIGH'] and count > 0:
                    report.append(f"  - Sample emails:\n")
                    for email in by_risk_level[level][:3]:  # Show max 3
                        report.append(f"    - From: {email['from']}, Subject: {email['subject']}\n")
                        report.append(f"      Risk Score: {email['risk_score']}, Reasons: {', '.join(email['reasons'])}\n")
