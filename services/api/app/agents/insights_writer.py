...
class InsightsWriterAgent:
    ...
    def plan(self, objective: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        )  # email_activity, applications, etc.
    ...
    def execute(self, plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        artifact_path = artifacts_store.get_weekly_path(
            prefix=report_type, extension="md"
        )
        ...
        json_path = artifacts_store.get_weekly_path(
            prefix=report_type, extension="json"
        )
        artifacts_store.write_json(json_path, json_artifact, agent_name=self.NAME)

        return {
            "report_type": report_type,
            "week": week_label,
            "artifacts": {"report": artifact_path, "data": json_path},
            "metrics_summary": {"current": current_metrics, "trends": trends},
            "ops_count": ops_count,
        }

    def _query_metrics(
        self, bq, report_type: str, year: int, week: int
    ) -> Dict[str, Any]:
    ...
    def _generate_report(
        ...
        # Header
        report.append(
            f"# Weekly Insights Report: {report_type.replace('_', ' ').title()}\n"
        )
        ...
        if report_type == "email_activity":
            ...
            report.append(
                f"- From **{current.get('unique_senders', 0):,}** unique senders\n"
            )
            report.append(f"- Spam rate: **{spam_pct:.1f}%** ({spam:,} spam emails)\n")
            report.append(
                f"- Emails with attachments: **{current.get('emails_with_attachments', 0):,}**\n\n"
            )

        elif report_type == "applications":
            report.append(
                f"- Submitted **{current.get('total_applications', 0):,}** applications\n"
            )
            report.append(
                f"- To **{current.get('unique_companies', 0):,}** unique companies\n"
            )
            report.append(
                f"- Scheduled **{current.get('interviews', 0):,}** interviews\n"
            )
        ...
        biggest_increase = max(
            trends.items(), key=lambda x: x[1]["change_pct"], default=None
        )
        biggest_decrease = min(
            trends.items(), key=lambda x: x[1]["change_pct"], default=None
        )
        ...
        if biggest_increase and biggest_increase[1]["change_pct"] > 10:
            ...
            report.append(
                f"- 📈 **{metric.title()}** increased significantly by **{pct:.1f}%**\n"
            )
