...
class OnlineLearningGuard:
    ...
    def auto_apply_approved_bundles(self, initial_canary_percent: int = 10):
        ...
        approved = [
            ...
            if self.db.query(
                ...
                .filter_by(key=f"approval.{a['id']}.status")
                .first()
            ).first()
        ]
        ...
        for approval in approved:
            try:
                self.bundle_mgr.apply_approved_bundle(
                    approval["id"], canary_percent=initial_canary_percent
                )
                deployed_count += 1
                logger.info(
                    f"Auto-deployed bundle {approval['bundle_id']} as {initial_canary_percent}% canary"
                )
    ...
    def check_canary_performance(
        self, agent: str, lookback_hours: int = 24, regression_threshold: float = 0.05
    ) -> Dict[str, any]:
        ...
        regression = self.detector.detect_regression(
            agent, since_hours=lookback_hours, threshold=regression_threshold
        )
        ...
        if (
            quality_delta > 0.02 or latency_delta < -0.1
        ):  # 2% quality improvement or 10% faster
            return {
    ...
    def promote_canary(self, agent: str, target_percent: int = 100):
    ...
    def gradual_rollout(
        self, agent: str, stages: list = [10, 50, 100], check_interval_hours: int = 24
    ) -> Dict[str, any]:
        ...
        # Check performance
        performance = self.check_canary_performance(
            agent, lookback_hours=check_interval_hours
        )
    ...
    def _update_canary_percent(self, agent: str, percent: int):
        ...
        if setting:
            ...
            setting = RuntimeSettings(
                key=key, value=str(percent), category="planner_canary"
            )
            self.db.add(setting)
