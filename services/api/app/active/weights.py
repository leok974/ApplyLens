...
class JudgeWeights:
    ...
    def update_weights_for_agent(
        self, agent: str, lookback_days: int = 30, decay_halflife_days: float = 7.0
    ) -> Dict[str, float]:
        ...
        for result in eval_results:
            ...
            for judge_name, score_data in judge_scores.items():
                ...
                confidence = (
                    score_data.get("confidence", 50) / 100.0
                )  # Normalize to 0-1
        ...
        for judge_name, metrics in judge_metrics.items():
            ...
            time_deltas = [
                (now - ts).total_seconds() / 86400.0 for ts in metrics["timestamps"]
            ]  # Days
    ...
    def _save_weights(self, agent: str, weights: Dict[str, float]):
        ...
        if setting:
            ...
            setting = RuntimeSettings(
                key=key, value=json.dumps(weights), category="active_learning"
            )
            self.db.add(setting)
    ...
    def update_all_agents(self, lookback_days: int = 30) -> Dict[str, Dict[str, float]]:
        ...
        for agent in agents:
            try:
                weights = self.update_weights_for_agent(
