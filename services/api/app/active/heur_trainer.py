...
class HeuristicTrainer:
    ...
    def train_for_agent(
        ...
        if len(examples) < min_examples:
            logger.info(
                f"Insufficient examples for {agent}: {len(examples)} < {min_examples}"
            )
            return None
        ...
        logger.info(
            f"Trained {model_type} for {agent}: {len(X)} examples, accuracy={bundle['accuracy']:.2f}"
        )
    ...
    def _extract_thresholds(
        self, agent: str, model, feature_importances: Optional[List[float]]
    ) -> Dict[str, Any]:
        ...
        if agent == "inbox_triage":
            ...
            if hasattr(model, "intercept_"):
                ...
                return {
                    ...
                    "spf_dkim_weight": round(abs(coeffs[1] + coeffs[2]), 2)
                    if len(coeffs) > 2
                    else 1.0,
                }
