...
class FeedLoader:
    ...
    def load_from_approvals(
        ...
        approvals = self.db.query(AgentApproval).filter(
            and_(
                AgentApproval.created_at >= since,
                AgentApproval.status.in_(["approved", "rejected"])
            )
        ).order_by(AgentApproval.created_at.desc()).limit(limit).all()
        
        count = 0
        for approval in approvals:
            # Check if already loaded
            exists = self.db.query(LabeledExample).filter_by(
                source="approvals",
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        recent = self.db.query(LabeledExample).filter(
            LabeledExample.created_at >= week_ago
        ).count()
