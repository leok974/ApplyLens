...
class BundleManager:
    ...
    def create_bundle(
        self, agent: str, min_examples: int = 50, model_type: str = "logistic"
        ...
        bundle = trainer.train_for_agent(
            agent, min_examples=min_examples, model_type=model_type
        )
    ...
    def propose_bundle(
        self, agent: str, bundle_id: str, proposer: str = "system"
    ) -> str:
        ...
        approval = AgentApproval(
            agent=agent,
            action="apply_bundle",
            context={"bundle_id": bundle_id, "diff": diff, "bundle": bundle},
            status="pending",
    ...
    def approve_bundle(
        self, approval_id: str, approver: str, rationale: Optional[str] = None
    ):
        ...
        logger.info(
            f"Approved bundle {approval.context['bundle_id']} for {approval.agent}"
        )

        # Note: Actual apply happens separately via apply_approved_bundle

    def apply_approved_bundle(
        self, approval_id: str, canary_percent: Optional[int] = None
    ):
        ...
        logger.info(
            f"Applied bundle {bundle_id} for {agent} (canary={canary_percent}%)"
        )
    ...
    def _save_bundle(self, agent: str, bundle: Dict[str, Any]):
        ...
        setting = RuntimeSettings(
            key=key, value=json.dumps(bundle), category="active_learning"
        )
    ...
    def _backup_active_bundle(self, agent: str):
        ...
        if setting:
            ...
            setting = RuntimeSettings(
                key=key, value=json.dumps(active), category="active_learning"
            )
            self.db.add(setting)
    ...
    def _apply_as_canary(self, agent: str, bundle: Dict[str, Any], canary_percent: int):
        ...
        canary_setting = (
            self.db.query(RuntimeSettings).filter_by(key=canary_key).first()
        )
        ...
        if canary_setting:
            ...
            canary_setting = RuntimeSettings(
                key=canary_key, value=str(canary_percent), category="planner_canary"
            )
