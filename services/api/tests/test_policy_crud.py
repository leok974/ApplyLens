...
def test_put_policy_roundtrip(client: TestClient):
    ...
    updated_policy = {
        ...
        "auto_unsubscribe_inactive": {"enabled": True, "threshold": 7},
    }
    ...
    assert (
        got["autoArchiveExpiredPromos"] == updated_policy["auto_archive_expired_promos"]
    )
    ...
    assert (
        persisted["autoQuarantineHighRisk"]
        == updated_policy["auto_quarantine_high_risk"]
    )
    assert (
        persisted["autoArchiveExpiredPromos"]
        == updated_policy["auto_archive_expired_promos"]
    )
...
def test_put_policy_partial_update(client: TestClient):
    ...
    assert (
        got["autoUnsubscribeInactive"]["enabled"]
        == base["autoUnsubscribeInactive"]["enabled"]
    )
    assert (
        got["autoUnsubscribeInactive"]["threshold"]
        == base["autoUnsubscribeInactive"]["threshold"]
    )
