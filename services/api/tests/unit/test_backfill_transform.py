...
def test_transform_adds_dates_and_expires():
    ...
    assert any(
        s.startswith("2025-10-15") for s in upd["dates"]
    ), "Should extract 10/15/2025"
...
def test_transform_respects_existing_earlier_expiry():
    ...
    if upd:
        ...
        assert (
            upd["expires_at"] == "2025-10-12T00:00:00Z"
...
def test_transform_updates_when_due_is_earlier_than_current_expires():
    ...
    assert upd["expires_at"].startswith(
        "2025-10-10"
    ), "Should update to earlier date Oct 10"
    assert any(
        s.startswith("2025-10-10") for s in upd["dates"]
    ), "Should include new date in dates array"
...
def test_transform_month_name_format():
    ...
    assert any(
        s.startswith("2025-12-25") for s in upd["dates"]
    ), "Should extract December 25, 2025"
...
def test_transform_multiple_dates():
    ...
    assert upd["expires_at"].startswith(
        "2025-10-15"
    ), "expires_at should be earliest (Oct 15)"
...
def test_transform_extracts_from_subject():
    ...
    d = _doc(body_text="Please pay your bill.", subject="Bill due October 20, 2025")
    upd = transform(d)

    assert upd is not None, "Should extract from subject"
    assert any(
        s.startswith("2025-10-20") for s in upd["dates"]
    ), "Should extract Oct 20 from subject"
...
def test_earliest_helper():
    ...
    assert (
        earliest(
            ["2025-10-20T00:00:00Z", "2025-10-15T00:00:00Z", "2025-10-25T00:00:00Z"]
        )
        == "2025-10-15T00:00:00Z"
    ), "Should return earliest date"
...
def test_transform_recomputes_expires_from_existing_dates():
    ...
    if upd:
        ...
        assert (
            upd["expires_at"] == "2025-10-15T00:00:00Z"
        ), "Should recompute expires_at to earliest"


def test_transform_updates_dates_adds_expires():
    """Test updating dates when expires_at doesn't exist."""
    d = _doc("Payment due by 10/20/2025", dates=None, expires_at=None)
    upd = transform(d)
