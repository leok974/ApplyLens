...
class TestApprovalsBulkInsert:
    ...
    def test_bulk_insert_rollback_on_error(self, monkeypatch):
        ...
        with patch("app.db.SessionLocal", mock_session_local):
            ...
            rows = [{"email_id": "e1", "action": "archive", "policy_id": "p1", "confidence": 0.9}]
...
class TestApprovalsGet:
    ...
    def test_get_proposed_status(self, monkeypatch):
        ...
        # Mock result
        mock_row = (1, "e1", "archive", "p1", 0.9, "expired", '{"x":1}', "proposed", "2025-10-10T00:00:00Z")
        
        mock_result = Mock()
        mock_result.keys = Mock(return_value=["id", "email_id", "action", "policy_id", "confidence", "rationale", "params", "status", "created_at"])
