...
class TestPolicyEngineBasics:
    ...
    def test_eval_clause_comparison(self):
        ...
        assert _eval_clause({"field": "risk_score", "op": ">", "value": 80}, obj) is True
        assert _eval_clause({"field": "risk_score", "op": ">=", "value": 85}, obj) is True
        assert _eval_clause({"field": "risk_score", "op": "<", "value": 90}, obj) is True
        assert _eval_clause({"field": "risk_score", "op": "<=", "value": 85}, obj) is True
        assert _eval_clause({"field": "risk_score", "op": ">", "value": 90}, obj) is False
    
    def test_eval_clause_contains(self):
        """Test contains operator."""
        obj = {"subject": "Special offer for you"}
        assert _eval_clause({"field": "subject", "op": "contains", "value": "offer"}, obj) is True
        assert _eval_clause({"field": "subject", "op": "contains", "value": "missing"}, obj) is False
    
    def test_eval_clause_in(self):
        """Test in operator."""
        obj = {"tags": ["urgent", "action-required"]}
        assert _eval_clause({"field": "tags", "op": "in", "value": ["urgent", "foo"]}, obj) is True
        assert _eval_clause({"field": "tags", "op": "in", "value": ["missing"]}, obj) is False
    
    def test_eval_clause_regex(self):
        """Test regex operator."""
        obj = {"subject": "Account verification required"}
        assert _eval_clause({"field": "subject", "op": "regex", "value": r"verif|confirm"}, obj) is True
        assert _eval_clause({"field": "subject", "op": "regex", "value": r"^urgent"}, obj) is False
...
class TestPolicyMatching:
    ...
    def test_policy_no_match(self):
        ...
        policies = [{
            ...
            "then": {"action": "archive", "confidence_min": 0.75, "rationale": "expired"}
        }]
        ...
        policies = [
            {
                ...
                "then": {"action": "archive", "confidence_min": 0.8, "rationale": "expired"}
            },
            {
                "id": "high-risk",
                "if": {"field": "risk_score", "op": ">=", "value": 80},
                "then": {"action": "quarantine", "confidence_min": 0.9, "rationale": "high risk"}
            }
        ]
