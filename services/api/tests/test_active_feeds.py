...
class TestFeedLoader:
    ...
    def test_get_stats(self, mock_db):
        ...
        mock_db.query.return_value.filter_by.return_value.count.side_effect = (
...
class TestLoadAllFeeds:
    ...
    def test_load_all_feeds(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.active.feeds.FeedLoader.__init__", lambda self, db: None
        )
        monkeypatch.setattr(
            "app.active.feeds.FeedLoader.load_from_approvals", lambda self: 10
        )
        monkeypatch.setattr(
            "app.active.feeds.FeedLoader.load_from_feedback", lambda self: 20
        )
        monkeypatch.setattr(
            "app.active.feeds.FeedLoader.load_from_goldsets", lambda self: 15
        )
