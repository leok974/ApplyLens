...
class EmailFactory(factory.Factory):
    ...
    gmail_id = factory.LazyAttribute(
        lambda obj: f"gmail_{obj.id}_{factory.Faker('uuid4')}"
    )
    ...
    sender = factory.LazyFunction(
        lambda: f"{factory.Faker('first_name')().lower()}@{factory.Faker('domain_name')()}"
    )
    recipient = "me@example.com"
    received_at = factory.LazyFunction(
        lambda: datetime.now()
        - timedelta(days=factory.Faker("random_int", min=0, max=30)())
    )
    ...
    source = factory.fuzzy.FuzzyChoice(
        ["linkedin", "indeed", "company_site", "referral"]
    )
    source_confidence = fuzzy.FuzzyFloat(0.5, 1.0)

    # Categorization
    category = factory.fuzzy.FuzzyChoice(
        ["personal", "promotions", "bills", "security", "applications"]
    )
...
class HighRiskEmailFactory(EmailFactory):
    ...
    quarantined = True
    flags = factory.LazyFunction(
        lambda: [
            {
                "signal": "suspicious_link",
                "evidence": "http://phishing.com",
                "weight": 0.8,
            },
            {
...
class ApplicationEmailFactory(EmailFactory):
    ...
    subject = factory.LazyAttribute(
        lambda obj: f"Application for {obj.role} at {obj.company}"
    )
...
def create_email_with_weights(
    user_id: str, positive_features: list[str], negative_features: list[str] = None
) -> tuple[Email, list[UserWeight]]:
    """
...
def create_policy_with_emails(
    n_emails: int = 3, **policy_kwargs
) -> tuple[Policy, list[Email]]:
    """
