...
CURRENT_SYNONYMS = [
    {"group": "job_action", "terms": "apply, submit"},  # Missing 'send'
    {
        "group": "job_doc",
        "terms": "cv, resume",
    },  # Different order, missing 'curriculum vitae'
...
def test_synonyms_diff_generation():
    ...
    print("\nDiff details:")
    print(
        f"- Added groups: {[item['group'] for item in diff_artifact['diff']['added']]}"
    )
    print(
        f"- Removed groups: {[item['group'] for item in diff_artifact['diff']['removed']]}"
    )
    ...
    assert not result["applied"], "Should not apply in dry-run"
...
def test_no_changes():
    ...
    factory = MockProviderFactory(
        bq_data={"synonyms": same_synonyms}, es_config={"synonyms": current_config}
    )
...
def test_dry_run_vs_live():
    ...
    # Dry-run mode
    plan_dry = agent.plan(
        "Update synonyms", {"config_type": "synonyms", "apply_changes": True}
    )
    ...
    # Assertions
    assert not result_dry["applied"], "Dry-run should not apply"
    ...
    if result_live["added_count"] <= 10 and result_live["removed_count"] <= 10:
        assert result_live["applied"], "Small changes should be approved"
...
def test_report_generation():
    ...
    plan = agent.plan("Update synonyms", {"config_type": "synonyms"})
    plan["dry_run"] = True
    plan["started_at"] = "2024-01-15T10:00:00Z"
    ...
    assert (
        result["artifacts"]["diff_report"] == "synonyms.diff.md"
...
def test_large_diff():
    ...
    large_mart = [
        {
            "term": f"term_{i}",
            "synonym_group": f"group_{i // 5}",
        }  # 5 terms per group, 20 groups
        for i in range(100)
    ]
    ...
    small_current = [
        {
            "group": f"group_{i}",
            "terms": ", ".join([f"term_{j}" for j in range(i * 5, (i + 1) * 5)]),
        }
        for i in range(5)
    ]

    factory = MockProviderFactory(
        bq_data={"synonyms": large_mart}, es_config={"synonyms": small_current}
    )
    ...
    assert (
        result["added_count"] >= 10
    ), f"Should detect large diff (got {result['added_count']} added)"
...
def test_config_key_uniqueness():
    ...
    syn2 = {
        "group": "job_action",
        "terms": "apply, submit, send",
    }  # Different terms, same group
