...
def test_risk_scorer_phishing():
    ...
    assert result['risk_score'] >= 80, f"Expected high risk score, got {result['risk_score']}"
...
def test_risk_scorer_legit():
    ...
    assert result['risk_score'] < 30, f"Expected low risk score, got {result['risk_score']}"
...
def test_risk_scorer_promo():
    ...
    assert result['risk_score'] < 40, f"Expected medium-low risk, got {result['risk_score']}"
    print("  ✓ PASSED")
...
def test_inbox_triage_live_mode():
    ...
    assert result['quarantined_count'] == 0, "Quarantine should be denied without approval"
    print("  ✓ PASSED")
...
def test_inbox_triage_risk_distribution():
    ...
    # Create plan
    plan = agent.plan(
        objective="Triage all emails",
        params={'max_emails': 100}
    )
    plan['dry_run'] = True
