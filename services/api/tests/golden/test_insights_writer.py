...
def test_email_activity_report():
    ...
    assert ',' in report and any(num in report for num in ['1,250', '1,100']), "Should format large numbers with commas"
    assert '📈' in report or '📉' in report or '➡️' in report, "Should have trend indicators"
    ...
    assert total_trend['current'] in [1250, 1100], f"Current should be valid value, got {total_trend['current']}"
    assert total_trend['previous'] in [1250, 1100], f"Previous should be valid value, got {total_trend['previous']}"
...
def test_weekly_artifact_paths():
    ...
    plan = agent.plan("Generate report", {'report_type': 'email_activity', 'week_offset': 0})
    plan['started_at'] = '2024-01-15T10:00:00Z'
    result = agent.execute(plan)
    ...
    # Assertions
    assert result['week'].startswith(('2024-W', '2025-W')), f"Should have ISO week format, got {result['week']}"
    assert 'email_activity' in result['artifacts']['report'], "Report should include report type"
...
def test_report_formatting():
    ...
    plan = agent.plan("Generate report", {'report_type': 'email_activity', 'include_charts': True})
    plan['started_at'] = '2024-01-15T10:00:00Z'
    result = agent.execute(plan)
...
def test_ops_counting():
    ...
    assert result['ops_count'] == 2, f"Should be 2 ops (query current + previous), got {result['ops_count']}"
...
def test_charts_optional():
    ...
    # Without charts
    plan_no_charts = agent.plan("Generate report", {'report_type': 'email_activity', 'include_charts': False})
    plan_no_charts['started_at'] = '2024-01-15T10:00:00Z'
    result_no_charts = agent.execute(plan_no_charts)
    
    report_path = f"agent/artifacts/{agent.NAME}/{result_no_charts['artifacts']['report']}"
    with open(report_path, 'r', encoding='utf-8') as f:
        report_no_charts = f.read()
    
    print(f"Report without charts has Visual Trends: {'Visual Trends' in report_no_charts}")
    assert 'Visual Trends' not in report_no_charts, "Should not have visual section when charts disabled"
    
    # With charts
    plan_with_charts = agent.plan("Generate report", {'report_type': 'email_activity', 'include_charts': True})
    plan_with_charts['started_at'] = '2024-01-15T10:00:00Z'
    result_with_charts = agent.execute(plan_with_charts)
    
    report_path2 = f"agent/artifacts/{agent.NAME}/{result_with_charts['artifacts']['report']}"
    with open(report_path2, 'r', encoding='utf-8') as f:
        report_with_charts = f.read()
    
    print(f"Report with charts has Visual Trends: {'Visual Trends' in report_with_charts}")
    assert 'Visual Trends' in report_with_charts, "Should have visual section when charts enabled"
...
def test_value_formatting():
    ...
    assert agent._format_value(99.9) == '99.9', "Should format floats < 1000 with decimal"
