...
class TestRiskTop3Endpoint:
    ...
    def test_risk_top3_returns_sorted_signals(self, mock_analyzer, mock_db_session):
        ...
        mock_email.raw = {
            "payload": {"headers": [{"name": "From", "value": "john@example.com"}]}
        }
    ...
    @patch("app.routers.security.ANALYZER")
    def test_risk_top3_handles_fewer_than_3_signals(
        self, mock_analyzer, mock_db_session
    ):
        ...
        mock_analyzer.analyze.return_value = RiskAnalysis(
            risk_score=10,
            flags=[RiskFlag(signal="NEW_DOMAIN", evidence="Domain is new", weight=10)],
            quarantined=False,
        )
        ...
        mock_analyzer.analyze.return_value = RiskAnalysis(
            ...
            flags=[
                RiskFlag(
                    ...
                    evidence="google.com is trusted",
                    weight=-15,
                ),
