...
def test_upsert_application_from_email():
    ...
    try:
        ...
        test_email_ids = db.query(Email.id).filter(Email.gmail_id.in_(["test_gmail_id_1", "test_gmail_id_2", "test_gmail_id_3"])).all()
        test_email_ids = [e[0] for e in test_email_ids]
        if test_email_ids:
            db.query(Application).filter(Application.last_email_id.in_(test_email_ids)).update({Application.last_email_id: None}, synchronize_session=False)
            db.flush()
        
        # Now delete test applications and emails
        db.query(Application).filter(Application.gmail_thread_id.in_(["test_thread_A", "test_thread_B", "test_newsletter"])).delete(synchronize_session=False)
        db.query(Email).filter(Email.gmail_id.in_(["test_gmail_id_1", "test_gmail_id_2", "test_gmail_id_3"])).delete(synchronize_session=False)
        db.commit()
        ...
        assert app.status == AppStatus.interview  # Should be interview due to label_heuristics
        assert app.thread_id == "test_thread_A"
        assert e.application_id == app.id
        
        print(f"✅ Test passed: Application {app.id} created and linked to email {e.id}")
...
def test_from_email_endpoint_autofill():
    ...
    try:
        ...
        db.query(Application).filter(Application.gmail_thread_id == "test-thread-autofill-123").delete(synchronize_session=False)
        db.commit()
        ...
        assert data["company"].lower() in ["openai", "careers", "careers team"], f"Unexpected company: {data['company']}"
        
        # Verify role was extracted
        assert "Engineer" in data["role"], f"Expected 'Engineer' in role, got: {data['role']}"
        
        # Verify source defaults to Email
        assert data["source"] in ["Email", "email"], f"Unexpected source: {data['source']}"
        
        print(f"✅ Test passed: /from-email endpoint auto-extracted: company={data['company']}, role={data['role']}, source={data['source']}")
