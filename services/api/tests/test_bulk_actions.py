...
def test_bulk_quarantine(client: TestClient, sample_emails: list[str]):
    ...
    response = client.post("/api/security/bulk/quarantine", json=ids_to_quarantine)
...
def test_bulk_release(client: TestClient, sample_emails: list[str], db: Session):
    ...
    response = client.post("/api/security/bulk/release", json=ids_to_test)
...
def test_bulk_rescan(client: TestClient, sample_emails: list[str]):
    ...
    response = client.post("/api/security/bulk/rescan", json=ids_to_rescan)
...
def test_bulk_quarantine_empty_list(client: TestClient):
    ...
    """
    response = client.post("/api/security/bulk/quarantine", json=[])
...
def test_bulk_quarantine_nonexistent_ids(client: TestClient):
    ...
    response = client.post("/api/security/bulk/quarantine", json=fake_ids)
...
def test_bulk_rescan_partial_failure(
    client: TestClient, sample_emails: list[str], db: Session
):
    """
    ...
    response = client.post("/api/security/bulk/rescan", json=mixed_ids)
...
def test_bulk_operations_preserve_other_fields(
    client: TestClient, sample_emails: list[str], db: Session
):
    """
