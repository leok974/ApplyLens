...
@pytest.mark.asyncio
async def test_nl_clean_promos_generates_actions(monkeypatch, async_client):
    ...
    r = await async_client.post(
        "/nl/run", json={"text": "clean my promos older than 7 days"}
    )
...
@pytest.mark.asyncio
async def test_nl_clean_promos_custom_days(monkeypatch, async_client):
    ...
    r = await async_client.post(
        "/nl/run", json={"text": "clean promos older than 14 days"}
    )
...
@pytest.mark.asyncio
async def test_nl_clean_promos_default_days(monkeypatch, async_client):
...
@pytest.mark.asyncio
async def test_nl_clean_promos_no_matches(monkeypatch, async_client):
...
@pytest.mark.asyncio
async def test_nl_clean_promos_multiple_emails(monkeypatch, async_client):
