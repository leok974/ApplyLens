...
@pytest.mark.asyncio
async def test_full_approvals_flow():
    ...
    with patch("app.db.approvals_bulk_insert", mock_bulk_insert), \
        ...
        async with AsyncClient(app=app, base_url="http://test") as ac:
            ...
            r = await ac.post("/approvals/propose", json=propose_payload)
            assert r.status_code == 200
            assert r.json()["accepted"] == 3
            
            # Step 2: List proposed items
            r = await ac.get("/approvals/proposed")
            assert r.status_code == 200
            items = r.json()["items"]
            assert len(items) == 3
            ...
            r = await ac.post("/approvals/approve", json={"ids": [id1, id2]})
            assert r.status_code == 200
            assert r.json()["updated"] == 2
            assert r.json()["status"] == "approved"
            
            r = await ac.post("/approvals/reject", json={"ids": [id3]})
            assert r.status_code == 200
            ...
            r = await ac.post("/approvals/execute", json=exec_payload)
            assert r.status_code == 200
            assert r.json()["applied"] == 2


@pytest.mark.asyncio
async def test_propose_empty_items():
    ...
    with patch("app.db.approvals_bulk_insert"), \
        ...
        async with AsyncClient(app=app, base_url="http://test") as ac:
            r = await ac.post("/approvals/propose", json={"items": []})
...
async def test_approve_empty_ids():
    ...
    with patch("app.db.approvals_update_status"), \
        ...
        async with AsyncClient(app=app, base_url="http://test") as ac:
            r = await ac.post("/approvals/approve", json={"ids": []})
            assert r.status_code == 400


@pytest.mark.asyncio
async def test_reject_empty_ids():
    ...
    with patch("app.db.approvals_update_status"), \
        ...
        async with AsyncClient(app=app, base_url="http://test") as ac:
            r = await ac.post("/approvals/reject", json={"ids": []})
            assert r.status_code == 400


@pytest.mark.asyncio
async def test_execute_empty_items():
    """Test that executing with no items returns zero applied."""
    with patch("app.logic.audit_es.emit_audit"):
        from app.main import app
        
        async with AsyncClient(app=app, base_url="http://test") as ac:
            r = await ac.post("/approvals/execute", json={"items": []})
            assert r.status_code == 200
            assert r.json()["applied"] == 0


@pytest.mark.asyncio
async def test_list_proposed_with_limit():
    ...
    with patch("app.db.approvals_get", mock_get):
        ...
        async with AsyncClient(app=app, base_url="http://test") as ac:
            ...
            r = await ac.get("/approvals/proposed?limit=50")
            assert r.status_code == 200
            items = r.json()["items"]
            assert len(items) == 50
...
@pytest.mark.asyncio
async def test_execute_splits_actions_by_type():
...
@pytest.mark.asyncio
async def test_propose_audit_to_elasticsearch():
    ...
    with patch("app.db.approvals_bulk_insert", mock_bulk_insert), \
        ...
        async with AsyncClient(app=app, base_url="http://test") as ac:
            r = await ac.post("/approvals/propose", json={
                "items": [
                    {
                        "email_id": "e1",
                        "action": "archive",
...
@pytest.mark.asyncio
async def test_approve_audit_to_elasticsearch():
    ...
    with patch("app.db.approvals_update_status", mock_update), \
        ...
        async with AsyncClient(app=app, base_url="http://test") as ac:
            r = await ac.post("/approvals/approve", json={"ids": [1, 2, 3]})
