...
async def test_nl_bills_explicit_date_format(monkeypatch):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        r = await ac.post(
            "/nl/run",
            json={"text": "show bills due by 10/15/2025"}
        )
...
async def test_nl_bills_named_month_format(monkeypatch):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        r = await ac.post(
            "/nl/run",
            json={"text": "bills due before Oct 20, 2025"}
        )
...
async def test_nl_bills_with_missing_money_amounts(monkeypatch):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        r = await ac.post(
            "/nl/run",
            json={"text": "bills due before Friday"}
        )
...
async def test_nl_bills_audit_contains_bill_metadata(monkeypatch):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        reminder_audit = next(
            (x for x in emitted if x.get("email_id") == "bill_audit_test"),
            None
        )
