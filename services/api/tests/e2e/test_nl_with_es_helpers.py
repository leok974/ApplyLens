...
@pytest.mark.asyncio
async def test_nl_clean_promos_with_es(monkeypatch):
...
@pytest.mark.asyncio
async def test_nl_unsubscribe_stale_with_es(monkeypatch):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        r = await ac.post("/nl/run", json={"text": "unsubscribe from stale newsletters"})
...
@pytest.mark.asyncio
async def test_nl_show_suspicious_with_es(monkeypatch):
...
async def test_nl_clean_promos_custom_days(monkeypatch):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        r = await ac.post("/nl/run", json={"text": "clean promos from the last 14 days"})
...
async def test_nl_unsubscribe_custom_days(monkeypatch):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        r = await ac.post("/nl/run", json={"text": "unsubscribe from newsletters older than 90 days"})
...
@pytest.mark.asyncio
async def test_nl_multiple_intents_separate_requests(monkeypatch):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        r1 = await ac.post("/nl/run", json={"text": "clean up my promotions"})
        assert r1.status_code == 200
        j1 = r1.json()
        assert j1["intent"] == "clean_promos"
        
        # Second request: show suspicious
        r2 = await ac.post("/nl/run", json={"text": "show suspicious emails"})
...
@pytest.mark.asyncio
async def test_nl_empty_results(monkeypatch):
...
@pytest.mark.asyncio
async def test_nl_large_result_set(monkeypatch):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        r = await ac.post("/nl/run", json={"text": "clean all my old promos"})
...
@pytest.mark.asyncio
async def test_nl_with_summarize_bills(monkeypatch):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        r = await ac.post("/nl/run", json={"text": "summarize my bills"})
...
@pytest.mark.asyncio
async def test_nl_intent_variations(monkeypatch):
...
@pytest.mark.asyncio
async def test_nl_error_handling_invalid_text(monkeypatch):
    """Test NL agent handles invalid/empty text gracefully."""
    
    async with AsyncClient(app=app, base_url="http://test") as ac:
        # Empty text
        r1 = await ac.post("/nl/run", json={"text": ""})
        assert r1.status_code in [200, 400, 422]
        
        # Gibberish
        r2 = await ac.post("/nl/run", json={"text": "asdfghjkl qwerty"})
