...
WITH avg_risk_anomalies AS (
  ...
  FROM `{{ env_var('BQ_PROJECT') }}.marts.mrt_risk_daily` a
  LEFT JOIN `{{ env_var('BQ_PROJECT') }}.ml.pred_avg_risk` p
    ON a.d = p.d
...
email_count_anomalies AS (
  ...
  FROM `{{ env_var('BQ_PROJECT') }}.marts.mrt_risk_daily` a
  LEFT JOIN `{{ env_var('BQ_PROJECT') }}.ml.pred_email_count` p
    ON a.d = p.d
  WHERE a.d >= DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY)
),
...
parity_ratio_anomalies AS (
  ...
  FROM `{{ env_var('BQ_PROJECT') }}.marts.mrt_parity_drift` a
  LEFT JOIN `{{ env_var('BQ_PROJECT') }}.ml.pred_parity_ratio` p
    ON a.d = p.d
  WHERE a.d >= DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY)
),
...
backfill_p95_anomalies AS (
  ...
  FROM `{{ env_var('BQ_PROJECT') }}.marts.mrt_backfill_slo` a
  LEFT JOIN `{{ env_var('BQ_PROJECT') }}.ml.pred_backfill_p95` p
    ON a.d = p.d
  WHERE a.d >= DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY)
)
