...
def infer_reason(doc: dict) -> str:
    ...
    if doc.get("list_unsubscribe"):
        return "Unsubscribe header"
    if re.search(r"(deal|sale|promo|coupon|% off)", subj, re.I):
        return "Promo keywords"
...
def es_index(doc: dict):
    rid = doc["id"]
    r = requests.post(
        f"{ES_URL}/{ES_INDEX}/_doc/{rid}?refresh=true",
...
def main():
    ...
    while resp.get("nextPageToken"):
        resp = (
            svc.users()
            .messages()
            .list(userId="me", q=query, maxResults=250, pageToken=resp["nextPageToken"])
            .execute()
        )
        msgs.extend(resp.get("messages", []))
        pages += 1
    ...
    for idx, m in enumerate(msgs, 1):
        ...
        msg = (
            svc.users().messages().get(userId="me", id=m["id"], format="full").execute()
        )
        ...
        if "@" in sender:
            sender_dom = (
                tldextract.extract(sender.split("@")[-1]).registered_domain or ""
            )
        ...
        is_promo = bool(
            re.search(
                r"(deal|sale|promo|coupon|% off)", subject + " " + body_text, re.I
            )
        )
    ...
    print(
        f"   BQ count: bq query --project_id={BQ_PROJECT} 'SELECT COUNT(*) FROM {BQ_DATASET}.{BQ_TABLE}'"
    )
