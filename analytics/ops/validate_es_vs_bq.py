...
def main():
    ...
    try:
        ...
        logger.info(f"  Delta:         {abs(es_count - bq_count):,} documents ({delta_pct}%)")
        logger.info(f"  Threshold:     {THRESHOLD_PCT}%")
        logger.info("")
        ...
        if delta_pct <= THRESHOLD_PCT:
            logger.info(f"✅ VALIDATION PASSED (delta {delta_pct}% <= {THRESHOLD_PCT}%)")
            return 0
        else:
            logger.error(f"❌ VALIDATION FAILED (delta {delta_pct}% > {THRESHOLD_PCT}%)")
            return 2
