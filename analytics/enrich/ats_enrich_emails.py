...
ATS_VIEW_PATH = os.getenv("ATS_VIEW_PATH", "/data/warehouse/vw_applications_enriched.parquet")
...
def fetch_warehouse_view() -> pd.DataFrame:
    ...
    if not os.path.exists(ATS_VIEW_PATH):
        ...
        return pd.DataFrame(columns=[
            "application_id", "system", "company", "email", 
            "stage", "last_stage_change", "interview_date"
        ])
...
def enrich_emails(df: pd.DataFrame) -> int:
    ...
    df["ghosting_risk"] = df.apply(
        lambda r: compute_ghosting_risk(r, now),
        axis=1
    )
